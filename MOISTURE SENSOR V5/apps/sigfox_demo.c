...
main(void)
{
	...
	///////////////////////START OF NEW CODE (1 OF 3)////////////////////////////////

	WDTCTL = WDTPW + WDTHOLD;                 // Stop WDT (redundant)
		  P1DIR |= 0x01;                            // P1.0 output
		  P2DIR |= 0b00100000;	//power supply for water sensor
		  P1DIR |= 0b00100000; //TX LED
		  ...
		  ADC12CTL0 = ADC12SHT02 + ADC12ON;         // Sampling time, ADC12 on
		    ...
		    TB0CTL |= TBSSEL_1 + MC_2 + TBCLR + TBIE;	// ACLK, contmode, clear TAR
		    TB0CTL |= ID_2;								// enable interrupt


		  bspLedClear(BSP_LED_1);	//turn off red LED

		  ///////////////////////END OF NEW CODE (1 OF 3)////////////////////////////////

	// Infinite loop
	for(;;)
	{

///////////////////////START OF NEW CODE (2 OF 3)////////////////////////////////

		ADC12CTL0 &= ~ADC12SC;                   // Stop sampling/conversion
	  __bis_SR_register(LPM3_bits + GIE);       // Enter LPM3, enable interrupts
	  __no_operation();                         // For debugger

		waterSensor =  0b0000001 & P1OUT;		//read and store value of red LED (1 - no water, 0 - water)

		if (waterStatus != waterSensor || waterSensor == 1){		//if the state of the water has changed, or the water is out (regardless of state change), trigger transmission sequence

			waterStatus = waterSensor;	//update previous door status to current door status

			if (waterSensor == 0){		//if port P2.4 has LOW input (0), there is plenty of water


				P1OUT |= 0b00100000;		//turn on LED connected to output port P1.5


				message[11] = 0x00;		//plenty of water
				err = SfxSendFrame(message, sizeof(message), NULL, NULL);		//transmit message to sigfox network that there is plenty of water
			...
			if (waterSensor == 1){		//if water is empty

				P1OUT |= 0b00100000;		//turn on LED connected to output port P1.5

				message[11] = 0x01;		//empty water
...
///////////////////////START OF NEW CODE (3 OF 3)////////////////////////////////


// Timer0_B5 Interrupt Vector (TAIV) handler
...
#endif
{
	switch(__even_in_range(TB0IV,14))
  {
    ...
    case 14:                   // overflow
    	P2OUT |= 0b00100000;	//turn on power supply to moisture sensor
...
#endif
{
  ...
  case  6:                                  // Vector  6:  ADC12IFG0
    if (ADC12MEM0 <= 0x1ff){                 // ADC12MEM = A0 < 0.125*AVcc (check if voltage is less than ~0.411V)
      P1OUT |= BIT0;                        // P1.0 = 1
    }
    else{
      P1OUT &= ~BIT0;                       // P1.0 = 0
	...
	ADC12CTL0 &= ~ADC12SC;                   // Stop sampling/conversion
	P2OUT &= ~0b00100000;		//turn off power supply to moisture sensor
